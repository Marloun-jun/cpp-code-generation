...
class ImprovedCodeGenerator:
    ...
    def save_model(self, path):
        # Сохранение модели в папку 1_my_cpp_nn_project
        # Создаем путь к папке 1_my_cpp_nn_project/models
        target_path = '1_my_cpp_nn_project/models'
        os.makedirs(target_path, exist_ok=True)
        # Сохраняем модель
        self.model.save(f'{target_path}/improved_code_generator.h5')
        print(f"✅ Улучшенная модель сохранена в: {target_path}/improved_code_generator.h5")

    def save_model_safe(self, path):
        # Безопасное сохранение модели в папку 1_my_cpp_nn_project
        # Создаем путь к папке 1_my_cpp_nn_project/models
        target_path = '1_my_cpp_nn_project/models'
        os.makedirs(target_path, exist_ok=True)
        # Сохраняем в формате .keras (рекомендуемый)
        self.model.save(f'{target_path}/improved_code_generator.keras')
        print(f"✅ Модель сохранена в формате .keras: {target_path}/improved_code_generator.keras")
        # Также сохраняем веса отдельно
        self.model.save_weights(f'{target_path}/improved_code_generator.weights.h5')
        print(f"✅ Веса сохранены отдельно: {target_path}/improved_code_generator.weights.h5")

def check_data_files():
    # Проверка существования данных
    required_files = [
        'data/train_desc.npy',
        'data/train_code_in.npy', 
        'data/train_code_out.npy',
        'data/test_desc.npy',
        'data/test_code_in.npy',
        'data/test_code_out.npy',
        'tokenizers/description_tokenizer.pkl',
        'tokenizers/code_tokenizer.pkl'
    ]
    missing_files = []
...
def run_preprocessing_if_needed():
    # Запуск предобработки если данные отсутствуют
    ...
    if missing_files:
        ...
        try:
            # Импортируем и запускаем предобработку
            ...
            # Сохраняем токенизаторы в 1_my_cpp_nn_project/tokenizers
            preprocessor.save_tokenizers('1_my_cpp_nn_project/tokenizers')
            # Сохраняем данные в numpy в папку 1_my_cpp_nn_project/data
            data_path = '1_my_cpp_nn_project/data'
            os.makedirs(data_path, exist_ok=True)
            train_desc, train_code_in, train_code_out = train_data
            test_desc, test_code_in, test_code_out = test_data
            np.save(f'{data_path}/train_desc.npy', train_desc)
            np.save(f'{data_path}/train_code_in.npy', train_code_in)
            np.save(f'{data_path}/train_code_out.npy', train_code_out)
            np.save(f'{data_path}/test_desc.npy', test_desc)
            np.save(f'{data_path}/test_code_in.npy', test_code_in)
            np.save(f'{data_path}/test_code_out.npy', test_code_out)
            print("✅ Предобработка завершена успешно!")
            print(f"📁 Данные сохранены в: {data_path}/")
            print(f"📁 Токенизаторы сохранены в: 1_my_cpp_nn_project/tokenizers/")
            return True
...
if __name__ == "__main__":
    ...
    if not run_preprocessing_if_needed():
        print("❌ Не удалось подготовить данные. Завершение работы.")
        exit(1)
    # Анализ данных
    analyze_training_data()
    # Загрузка данных
    train_data, test_data, desc_tokenizer, code_tokenizer = load_training_data()
    train_desc, train_code_in, train_code_out = train_data
    test_desc, test_code_in, test_code_out = test_data
