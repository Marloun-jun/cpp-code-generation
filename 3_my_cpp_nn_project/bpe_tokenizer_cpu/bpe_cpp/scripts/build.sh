...
#
#           - **Типы сборки**:            Debug, Release, RelWithDebInfo, MinSizeRel
#           - **Очистка**:                полное удаление директории сборки
#           - **Тестирование**:           запуск модульных тестов после сборки
#           - **Бенчмарки**:              запуск тестов производительности
#           - **Установка**:              копирование файлов в директорию install
#           - **Параллельная сборка**:    автоматическое определение числа ядер
#           - **Генераторы**:              поддержка Make, Ninja
#
#          **Процесс работы:**
...
if command -v g++ &> /dev/null; then
    ...
    print_success "clang++ найден: $(clang++ --version | head -n1)"
else
    print_warning "Ни g++, ни clang++ не найдены. Убедитесь, что компилятор C++ установлен."
    TOOLS_OK=1
fi
...
# Добавление опций для разных компиляторов
if [[ "$CXX" == *"g++"* ]] || [[ -z "$CXX" && $(command -v g++) ]]; then
    # GCC специфичные опции
    CMAKE_OPTIONS+=("-DCMAKE_CXX_COMPILER=g++")
elif [[ "$CXX" == *"clang++"* ]] || [[ -z "$CXX" && $(command -v clang++) ]]; then
    # Clang специфичные опции
