 ...
 * 
 *          **Базовые операции**      - кодирование и декодирование примеров C++ кода
 *          **Пакетная обработка**    - сравнение последовательной и пакетной обработки
 *          **Кэширование**           - демонстрация эффективности кэша на повторяющихся паттернах
 *          **Производительность**    - тесты с разными размерами входных данных
 *          **SIMD оптимизации**      - проверка поддержки и влияние на скорость
 *          **Статистика**            - сбор и отображение метрик работы токенизатора
 * 
 *          Программа поддерживает аргументы командной строки:
 *          `--verbose` / `-v`    - подробный вывод
 *          `--quick` / `-q`      - пропустить медленные тесты
...
namespace {
    ...
    constexpr size_t DEMO_CACHE_SIZE = 10000;
    constexpr int WIDTH = 60;
    
    // Цвета для вывода (опционально, можно отключить если не поддерживаются)
...
/**
 * @brief RAII таймер для измерения времени выполнения
 * 
 ...
 * 
 * @param size_kb Желаемый размер в килобайтах
...
void demo_sse42_features() {
    ...
    double std_time;
    {
        ...
        std_time = timer.elapsed_ms() / 1000.0;
    }
    ...
    {
        ScopedTimer timer("SIMDUtils::strings_equal_sse42", true, false);
        for (int i = 0; i < 100000; ++i) {
...
void demo_direct_simd_encode() {
    ...
    std::cout << "  AVX2:   " << (avx2_available ? GREEN + "✓" : YELLOW + "○") << RESET << "\n";
    std::cout << "  AVX:    " << (avx_available ? GREEN + "✓" : YELLOW + "○") << RESET << "\n";
    std::cout << "  SSE4.2: " << (sse42_available ? GREEN + "✓" : YELLOW + "○") << RESET << "\n\n";
    ...
    if (avx2_available) {
        ...
        std::cout << "Результаты совпадают: " 
                  << (scalar_result.size() == avx2_result.size() ? GREEN + "✓" : RED + "✗") 
                  << RESET << "\n";
        
        // Вычисляем ускорение
        // Здесь нужно было бы измерить время, но у нас уже есть таймеры
    } else {
    ...
    if (avx_available) {
        ...
        std::cout << "Результаты совпадают: " 
                  << (scalar_result.size() == avx_result.size() ? GREEN + "✓" : RED + "✗") 
                  << RESET << "\n";
    } else {
        std::cout << YELLOW << "AVX encode недоступен\n" << RESET;
    }
    
    // ===== Демонстрация SSE4.2 поиска подстроки =====
    if (sse42_available) {
        std::cout << "\n------------------------------------------------------------\n";
        std::cout << "Тест SSE4.2 поиска подстроки:\n";
        
        std::string long_text = text + text + text;
        std::string pattern = "SIMD";
        
        size_t pos_std = 0;
        {
            ScopedTimer timer("std::string::find", true, false);
            for (int i = 0; i < 1000; ++i) {
                pos_std = long_text.find(pattern);
            }
        }
        ...
        size_t pos_simd = 0;
        {
            ScopedTimer timer("SIMDUtils::find_substring_sse42", true, false);
            for (int i = 0; i < 1000; ++i) {
                pos_simd = SIMDUtils::find_substring_sse42(long_text, pattern);
            }
        }
        
        std::cout << "Позиция (std): " << pos_std << "\n";
        std::cout << "Позиция (SIMD): " << pos_simd << "\n";
        std::cout << "Результаты совпадают: " 
                  << (pos_std == pos_simd ? GREEN + "✓" : RED + "✗") << RESET << "\n";
    }
}

/**
 * @brief Класс для поиска файлов модели
 */
class ModelFinder {
private:
...
public:
    ModelFinder() {
        ...
        // Модели в текущей директории (приоритет 3)
        candidates_.emplace_back("models/bpe_8000/cpp_vocab.json", 
                                 "models/bpe_8000/cpp_merges.txt");
        candidates_.emplace_back("models/bpe_10000/cpp_vocab.json", 
                                 "models/bpe_10000/cpp_merges.txt");
    ...
    bool find(FastBPETokenizer& tokenizer, std::string& vocab_path, std::string& merges_path) {
        ...
        std::cout << "  - bpe_cpp/models/bpe_8000/" << std::endl;
        std::cout << "  - bpe_python/models/bpe_8000/" << std::endl;
        return false;
    }
};
...
void demo_batch_processing(FastBPETokenizer& tokenizer, 
    ...
    std::cout << "Корректность: " << (all_match ? GREEN + "✓" : RED + "✗") 
              << RESET << std::endl;
}
