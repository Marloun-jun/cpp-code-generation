...
# @brief Пример использования C++ BPE токенизатора в PyTorch DataLoader
#
# @author Евгений П.
# @date 2026
# @version 3.2.0
#
# @details Демонстрирует интеграцию C++ токенизатора с PyTorch DataLoader:
...
CPP_BUILD_DIR = PROJECT_ROOT / 'cpp' / 'build'     # cpp/build/
...
if CPP_BUILD_DIR.exists():
    print(f"Файлы в C++ build директории:")
    for f in CPP_BUILD_DIR.iterdir():
        if f.name.endswith('.so') or 'bpe_tokenizer_cpp' in f.name:
            print(f"   • {f.name}")
else:
    print(f" !!! C++ build директория не найдена: {CPP_BUILD_DIR}")
...
class CppCodeDataset(Dataset):
    ...
    def __init__(self, data_path: str, max_length: int = 512, vocab_size: int = 32000):
        ...
        vocab_path = PROJECT_ROOT / 'bpe' / 'vocab_trained.json'
        merges_path = PROJECT_ROOT / 'bpe' / 'merges_trained.txt'
    ...
    def __getitem__(self, idx: int) -> torch.Tensor:
        ...
        Returns:
            torch.Tensor: Тензор с ID токенов
        """
        text = self.texts[idx]
...
def main() -> int:
    ...
    # Создаем датасет
    dataset = CppCodeDataset(str(data_path), max_length=256)
    ...
    print(f"\n{'=' * 70}")
    print(f"{'Конфигурация':<20} {'Batch':<8} {'Workers':<8} {'Скорость':<15} {'Время/батч':<15}")
    print(f"{'-' * 70}")
    
    for name, batch, workers, speed, time_per_batch in results:
        print(f"{name:<20} {batch:<8} {workers:<8} {speed:<15.0f} {time_per_batch:<15.2f} ms")
    ...
    if torch.cuda.is_available():
        ...
        print("Измерение скорости с GPU...")
        start_time = time.time()
