    ...
    def parse_csv_step_by_step(self, line: str) -> Optional[List[str]]:
        ...
        **Алгоритм:**
        ...
        try:
            ...
            if self.debug_mode:
                print(f"X Ошибка пошагового парсинга: {str(e)}")
                import traceback
                traceback.print_exc()
            return None
    ...
    def load_dataset(self, filename: str, max_lines: Optional[int] = None) -> Tuple[List[Dict], Dict]:
        ...
        try:
            ...
            if not filepath.exists():
                print(f"X Файл не найден: {filename}")
            ...
            if content is None:
                print("X Не удалось определить кодировку файла")
                return [], defaultdict(lambda: {'using': [], 'explicit': []})
            ...
            print(f"X Критическая ошибка загрузки: {str(e)}")
            import traceback
            traceback.print_exc()
            return [], defaultdict(lambda: {'using': [], 'explicit': []})
    ...
    **Важно:** Класс ТОЛЬКО проверяет код, не внося никаких исправлений.
    ...
    def _find_compiler(self) -> Optional[str]:
        ...
        if self.compiler_type not in self.COMPILER_VERSIONS:
            print(f"X Неизвестный тип компилятора: {self.compiler_type}")
            return None
    ...
    def _print_compiler_info(self) -> bool:
        ...
        if not self.compiler_path:
            print(f"X Компилятор {self.compiler_type} не найден")
        ...
        try:
            ...
            print(f"X Ошибка проверки компилятора: {str(e)}")
            return False
...
def main() -> None:
    ...
    print_header("ПРОДВИНУТЫЙ ТЕСТЕР КОМПИЛЯЦИИ C++ КОДА v3.0")  
    print("Особенности:")
    ...
    try:
        ...
        if not compiler.compiler_path:
            print("X Не удалось найти компилятор. Программа завершена.")
            return
                ...
                if choice == 'q' or choice == 'й':
                    ...
                    try:
                        ...
                        if 1 <= topic_num <= len(topics):
                            ...
                            print("X Неверный номер темы")
                    
                    except ValueError:
                        print("X Неверный ввод. Введите число.")
                    ...
                    if not lines_input:
                        print("X Не указаны номера строк")
                        continue
                    ...
                    try:
                        ...
                        if selected_examples:
                            ...
                            print("X Не найдено ни одного примера по указанным строкам")
                    
                    except ValueError:
                        print("X Неверный формат ввода. Используйте формат: 100,150,200")
                
                else:
                    print("X Неверный выбор. Пожалуйста, введите 1-5, D или Q")
