    ...
    try:
        with open(filename, 'r', encoding='utf-8') as f:
            ...
            for line_num, line in enumerate(lines, 1):
                ...
                # проверяем результат
                if line_num <= 3:
                    print(f"Исправленный код (первые 5 строк):")
                    lines_code = fixed_code.split('\n')
                    for j, code_line in enumerate(lines_code[:5]):
                        print(f"  Строка {j+1}: {code_line}")
                    if len(lines_code) > 5:
                        print(f"  ... и еще {len(lines_code)-5} строк")
                example = {
                    'line': line_num,
...
def main():
    ...
    while True:
        try:
            ...
            # if not choice:  # если пустая строка
                #print("❌ Некорректный выбор. Пожалуйста, введите 1, 2, 3 или q.")
                #continue
            if choice.lower() == 'q':
                print("Выход...")
                break
