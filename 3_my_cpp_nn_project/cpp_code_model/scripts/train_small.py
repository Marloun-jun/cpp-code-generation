...
# -*- coding: utf-8 -*-
# ======================================================================
# train_small.py - Обучение CppCodeModel-Small (8.3M параметров)
# ======================================================================
#
# @file train_small.py
# @brief Скрипт для обучения средней модели C++ кода с 8,3M параметров
#
# @author Евгений П.
# @date 2026
# @version 3.2.0
#
# @details Этот скрипт обучает модель CppCodeModel-Small с 8.3M параметров, 
#          оптимизированную для GPU с 4GB VRAM (например, GTX 1650), 
#          для датасета из 9.3K примеров.
#          Модель имеет баланс между качеством и требованиями к памяти.
#
#          **Основные возможности:**
#
#          1. **Оптимизированная конфигурация для 4GB VRAM**
#             - d_model=256
#             - nhead=4
#             - num_layers=4
#             - max_len=768
...
#
# @usage
#     python scripts/train_small_gpu.py
#
# @example
#     # Стандартный запуск
#     python train_small_gpu.py
#
#     # С указанием конкретного GPU
#     CUDA_VISIBLE_DEVICES=0 python train_small_gpu.py
#
# ======================================================================
...
# ======================================================================
# КОНФИГУРАЦИЯ МОДЕЛИ SMALL (8.3M)
# ======================================================================

config = {
    "name": "CppCodeModel-Small",
    "description": "8.3M параметров",
    "vocab_size": 10001,
...
CHECKPOINT_DIR = PROJECT_ROOT / "checkpoints" / "small"
...
with open(CONFIG_DIR / "small.json", 'w', encoding='utf-8') as f:
...
print("=" * 60)
print("ОБУЧЕНИЕ CppCodeModel-Small (8.3M)")
print("=" * 60)
...
print(f"- Параметров:    ~8.3M")
print(f"- Batch size:    {config['batch_size']}")
print(f"- Max length:    {config['max_len']} (было 1024)")
...
print("\nПРОГНОЗ ВРЕМЕНИ:")
if torch.cuda.is_available():
    print("GPU (GTX 1650): 2-3 часа")
else:
    print("CPU: 15-20 часов")

print("\nПУТИ:")
print(f"- Данные: {DATA_DIR}")
print(f"- Чекпоинты: {CHECKPOINT_DIR}")
print(f"- Конфиг: {CONFIG_DIR / 'small.json'}")
print("=" * 60)
...
callbacks = [
    pl.callbacks.ModelCheckpoint(
        ...
        filename='cpp-code-small-{epoch:02d}-{val_loss:.2f}',
        monitor='val_loss',
        mode='min',
        save_top_k=3,
        save_last=True
